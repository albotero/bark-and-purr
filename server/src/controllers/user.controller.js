...
export const updateAvatar = async (req, res) => {
  ...
  try {
    ...
    const optimizedImage = await sharp(file.buffer).resize(300).jpeg({ quality: 80 }).toBuffer()
    ...
    await connectionDb.query("UPDATE users SET avatar_url = $1, avatar_key = $2 WHERE id = $3", [
      avatar_url,
      avatar_key,
      id,
    ])
